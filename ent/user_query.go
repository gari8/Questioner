...
func (uq *UserQuery) QueryQuestions() *QuestionQuery {
	query := &QuestionQuery{config: uq.config}
...
func (uq *UserQuery) QueryAnswers() *AnswerQuery {
	query := &AnswerQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryChoiceanswers() *ChoiceAnswerQuery {
	query := &ChoiceAnswerQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withChoiceanswers: uq.withChoiceanswers.Clone(),
...
func (uq *UserQuery) WithChoiceanswers(opts ...func(*ChoiceAnswerQuery)) *UserQuery {
	query := &ChoiceAnswerQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withChoiceanswers = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withQuestions; query != nil {
		...
		query.Where(predicate.Question(func(s *sql.Selector) {
			s.Where(sql.InValues(user.QuestionsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_questions
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_questions" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_questions" returned %v for node %v`, *fk, n.ID)
			}
	...
	if query := uq.withAnswers; query != nil {
		...
		query.Where(predicate.Answer(func(s *sql.Selector) {
			s.Where(sql.InValues(user.AnswersColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_answers
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_answers" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_answers" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Answers = append(node.Edges.Answers, n)
		}
	}

	if query := uq.withChoiceanswers; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[string]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.ChoiceAnswer(func(s *sql.Selector) {
			s.Where(sql.InValues(user.ChoiceanswersColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_choiceanswers
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_choiceanswers" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_choiceanswers" returned %v for node %v`, *fk, n.ID)
			}
