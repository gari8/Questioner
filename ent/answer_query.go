...
func (aq *AnswerQuery) QueryOwner() *UserQuery {
...
func (aq *AnswerQuery) QueryParent() *QuestionQuery {
...
func (aq *AnswerQuery) WithOwner(opts ...func(*UserQuery)) *AnswerQuery {
	query := &UserQuery{config: aq.config}
...
func (aq *AnswerQuery) WithParent(opts ...func(*QuestionQuery)) *AnswerQuery {
	query := &QuestionQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
...
//
//	client.Answer.Query().
//		GroupBy(answer.FieldAnswerType).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
func (aq *AnswerQuery) sqlAll(ctx context.Context) ([]*Answer, error) {
	...
	if aq.withOwner != nil || aq.withParent != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := aq.withOwner; query != nil {
		...
		nodeids := make(map[string][]*Answer)
		for i := range nodes {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_answers" returned %v`, n.ID)
			}
			for i := range nodes {
