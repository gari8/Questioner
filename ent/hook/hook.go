...
// The ChoiceAnswerFunc type is an adapter to allow the use of ordinary
// function as ChoiceAnswer mutator.
type ChoiceAnswerFunc func(context.Context, *ent.ChoiceAnswerMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ChoiceAnswerFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.ChoiceAnswerMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ChoiceAnswerMutation", m)
	}
	return f(ctx, mv)
}
